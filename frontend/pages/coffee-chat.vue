...
function getStatusClasses(status: string) {
  const classes: Record<string, string> = {
    PENDING: 'border-yellow-500/50 bg-yellow-50 text-yellow-700 dark:bg-yellow-900/20 dark:text-yellow-400',
    ACCEPTED: 'border-green-500/50 bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400',
    DECLINED: 'border-red-500/50 bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400',
    CANCELLED: 'border-gray-500/50 bg-gray-50 text-gray-500 dark:bg-gray-900/20 dark:text-gray-400',
  }
  ...
  <div class="max-w-2xl mx-auto py-8 px-4">
    ...
    <div class="flex items-center gap-3 mb-6">
      <div class="flex items-center justify-center h-10 w-10 rounded-lg bg-primary/10">
        <Icon icon="heroicons:chat-bubble-left-right" class="h-5 w-5 text-primary" />
      </div>
      <div>
        <h1 class="text-xl font-bold text-foreground">커피챗</h1>
        <p class="text-sm text-muted-foreground">다른 개발자와 1:1 대화를 나눠보세요</p>
      </div>
    </div>
    ...
    <div class="flex border-b border-border mb-6">
      ...
      >
        <Icon icon="heroicons:inbox-arrow-down" class="w-4 h-4 mr-1.5 inline-block align-text-bottom" />
        받은 요청
      </button>
      <button
        class="px-4 py-2.5 text-sm font-medium transition-colors border-b-2 -mb-px"
      ...
      >
        <Icon icon="heroicons:paper-airplane" class="w-4 h-4 mr-1.5 inline-block align-text-bottom" />
        보낸 요청
      </button>
    </div>
    ...
    <div v-if="loading" class="flex items-center justify-center py-16">
      <Icon icon="heroicons:arrow-path" class="h-6 w-6 animate-spin text-muted-foreground" />
    </div>
    ...
    <div v-else-if="chats.length === 0" class="text-center py-16">
      <div class="flex items-center justify-center h-16 w-16 rounded-full bg-muted mx-auto mb-4">
        <Icon icon="heroicons:chat-bubble-left-right" class="h-8 w-8 text-muted-foreground" />
      </div>
      <p class="text-sm text-muted-foreground">
        {{ activeTab === 'received' ? '아직 받은 커피챗 요청이 없습니다' : '아직 보낸 커피챗 요청이 없습니다' }}
      </p>
      <p class="text-xs text-muted-foreground mt-1">
        {{ activeTab === 'sent' ? '다른 개발자의 포트폴리오에서 커피챗을 요청해보세요' : '다른 개발자가 커피챗을 요청하면 여기에 표시됩니다' }}
      </p>
    </div>
    ...
    <div v-else class="space-y-3">
      <div
        v-for="chat in chats"
        :key="chat.id"
        class="border border-border rounded-xl bg-card p-4 transition-colors"
        :class="{ 'opacity-60': chat.status === 'DECLINED' || chat.status === 'CANCELLED' }"
      >
        <!-- 상단: 사용자 정보 + 상태 뱃지 -->
        <div class="flex items-start justify-between gap-3">
          <div class="flex items-center gap-3 min-w-0">
            <Avatar class="h-10 w-10 flex-shrink-0">
              ...
              <AvatarFallback>{{ getUserInitials(getOtherUser(chat)) }}</AvatarFallback>
            </Avatar>
            <div class="min-w-0">
              ...
              <!-- 기술 스택 태그 -->
              <div v-if="getOtherUser(chat)?.techStack?.length" class="flex flex-wrap gap-1 mt-1">
                <span
          ...
          <span class="text-[11px] text-muted-foreground whitespace-nowrap flex-shrink-0">
        ...
        <div v-if="chat.status === 'ACCEPTED' && chat.meetingUrl" class="mt-3">
          <div class="flex items-center gap-2 px-3 py-2 rounded-md bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
            <Icon icon="heroicons:link" class="h-4 w-4 text-green-600 dark:text-green-400 flex-shrink-0" />
            <a
              :href="chat.meetingUrl"
              target="_blank"
              rel="noopener noreferrer"
              class="text-sm text-green-700 dark:text-green-400 hover:underline truncate"
            >
              {{ chat.meetingUrl }}
            </a>
          </div>
        ...
        <div class="mt-3 flex items-center gap-2">
          <!-- 받은 요청 + PENDING: 수락/거절 -->
          <template v-if="activeTab === 'received' && chat.status === 'PENDING'">
            <Button
              size="sm"
              class="h-8"
    ...
    <!-- 수락 다이얼로그 (미팅 URL 입력) -->
    <Dialog :open="acceptDialogOpen" @update:open="(v) => acceptDialogOpen = v">
      <DialogContent class="sm:max-w-md">
        ...
        <div class="space-y-3">
          <div>
            <label class="text-xs text-muted-foreground mb-1 block">미팅 링크 (선택)</label>
            <Input
        ...
        <DialogFooter>
          <Button variant="outline" :disabled="acceptLoading" @click="acceptDialogOpen = false">
            취소
          </Button>
          <Button :disabled="acceptLoading" @click="handleAccept">
            <Icon v-if="acceptLoading" icon="heroicons:arrow-path" class="w-4 h-4 mr-2 animate-spin" />
            수락하기
          </Button>
        </DialogFooter>
      </DialogContent>
